...
dataset = DatasetDict({
    "train": load_dataset("json", data_files={"train": "dummy.json"}, split="train").map(format_example)
}) if False else DatasetDict({
    "train": DatasetDict.from_dict({"text": [format_example(x)["text"] for x in data["train"]]})
})
